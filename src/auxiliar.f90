...
do ifile=1,split_parts
	...
	if (endianness_data .eq. endianness_machine) then
	    ! Datos y máquina tienen el mismo endianness
	    open(unit0+ifile, FILE=file_name, ACCESS=access, FORM=form, STATUS='old')
	else
	    ! Ditinto endianness, se necesita conversión
	    if (endianness_data == LITTLE_ENDIAN) then
	        open(unit0+ifile, FILE=file_name, ACCESS=access, FORM=form, CONVERT='LITTLE_ENDIAN', STATUS='old')
	    else
	        open(unit0+ifile, FILE=file_name, ACCESS=access, FORM=form, CONVERT='BIG_ENDIAN', STATUS='old')
	    endif
	endif
	...
	if(split_parts.eq.1)file_name = trim(adjustl(su_file_DC0)) // ".su"
        ...
        file_name = trim(adjustl(folder_data_parts)) // trim(adjustl(file_name))
...
do ifile=1,split_parts
        ...
        file_name = trim(adjustl(folder_data_parts)) // trim(adjustl(file_name1))
        open(unit_PG1+ifile,FILE=file_name,ACCESS=access,FORM=form,CONVERT='BIG_ENDIAN',STATUS='unknown')

        file_name = trim(adjustl(folder_data_parts)) // trim(adjustl(file_name2))
        open(unit_PG2+ifile,FILE=file_name,ACCESS=access,FORM=form,CONVERT='BIG_ENDIAN',STATUS='unknown')

        file_name = trim(adjustl(folder_data_parts)) // trim(adjustl(file_name3))
...
character(len=500) :: command,Str_type,Str_NumRec,Str_nt,Str_ed_data,Str_ed_machine
        ...
        write(Str_type,*) data_type
        write(Str_NumRec,*) NumRec
        write(Str_nt,*) nt
        write(Str_ed_data,*) endianness_data
        write(Str_ed_machine,*) endianness_machine

        command="bash DC_in.sh " // trim(adjustl(folder_input))
        command= trim(adjustl(command)) // " " // trim(adjustl(Str_type)) !!new
        command= trim(adjustl(command)) // " " // trim(adjustl(data_file))
        command= trim(adjustl(command)) // " " //  trim(adjustl(Str_NumRec))
        command= trim(adjustl(command)) // " " //  trim(adjustl(Str_nt))
        command= trim(adjustl(command)) // " " //  trim(adjustl(Str_ed_data))
        command= trim(adjustl(command)) // " " //  trim(adjustl(Str_ed_machine))

        print *, 'command: ', trim(adjustl(command))

        call system(trim(command))

end subroutine Data_in

subroutine Data_out(iDC,Str_out)
USE mod_parfile

implicit none

integer :: iDC
character(len=500) :: command,Str_DC,Str_out

        write(*,*)
        write(*,*)'*************************************************************'
        write(*,*)'JOINT SU PARTS AND CREATING SGY OUTPUT FILE for step: ',iDC
        write(*,*)'*************************************************************'
        write(*,*)

        write(Str_DC,*) iDC

        command="bash DC_out.sh " // trim(adjustl(folder_data_parts))
        command= trim(adjustl(command)) // " " // trim(adjustl(Str_out))
        command= trim(adjustl(command)) // " " // trim(adjustl(Str_DC))
