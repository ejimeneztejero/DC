...
!!if(icount.le.nsamples.and.shotID_(icount).ne.0)     then

if(icount.le.nsamples)     then

if((iDC.eq.2).or.(iDC.eq.1.and.shotID_(icount).ne.0)) then
...
call DC_propagation(rank,iDC,nSSS,datum_i,datum_f,pos_grid,input,output)
...
subroutine DC_propagation(rank,iDC,nSS,&
...
!!---------- PROPAGATOR FOR A SUPERSHOT
	...
	call solver(rank,iDC,nSS,nxx,nys,nyr,source,ny_DC,nx_DC,model,output)
