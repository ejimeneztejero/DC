subroutine BP_filter(nSS,nt,dt,fa,fb,output)
implicit none

        integer :: j,nSS,nt,nt2,zero_pad,typef
        real :: dt,fe,Data_trace(nt),fa,fb
	...
	do j=1,nSS
		...
		Data_trace(:)=output(:,j)
	        ...
	        tmp=0.
	        tmp((zero_pad-1)*nt+1:nt2)=Data_trace(:)

	        call Butter_filter(tmp,nt2,fe,1,typef,fa,fb)
	        Data_trace(:)=tmp(nt2-nt+1:nt2)
...
subroutine Butter_filter(S,ts,fe,Num,typef,f1,fc)
implicit none

	integer :: i,j,ts,n,typef
	integer :: lensav,IER,lenwrk,parity,ind,Num
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	n=6! Filter order
	parity=0 ! parity=0 if n is even and 1 if n is odd, just useful in Butterworth

	!fc : Cut-off frecuency (in Hz) do low-pass
	fs=40! Start frecuency for high-pass (in Hz)
	...
	call Butterworth_function(n,parity,fc,fs,f1,f2,f,fe,f0,Q,ts,typef,H)
...
end subroutine Butter_filter


subroutine Butterworth_function(n,parity,fc,fs,f1,f2,f,fe,f0,Q,ts,typef,H)
implicit none

	integer :: i,j,k
	integer :: typef,ts,n,parity
	real, parameter :: pi = 3.14159265
	real :: f, fc, fs, f1, f2, fe, f0, Q 
	complex	:: H(1:ts)
	complex, parameter :: imag=(0.,1.)
	complex, allocatable :: Bn(:)
	allocate(Bn(1:ts))
...
end subroutine Butterworth_function
